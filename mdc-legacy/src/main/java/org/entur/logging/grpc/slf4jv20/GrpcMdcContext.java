...
import org.entur.logging.grpc.GrpcTraceMdcContext;
...
public class GrpcMdcContext {
	...
	public static class ContextWrapper {
		...
		private GrpcTraceMdcContext grpcTraceMdcContext;
		...
		public ContextWrapper() {
			...
			if (grpcMdcContext != null) {
				...
				if (copyOfContextMap != null) {
					this.fields = new HashMap<>(copyOfContextMap);
				} else {
		...
		public ContextWrapper withCorrelationId(String correlationId) {
			grpcTraceMdcContext = GrpcTraceMdcContext.newBuilder().withCorrelationId(correlationId).build();
			return this;
		}
		...
		private void ensureTracing() {
			// so tracing headers are always present
			if (grpcTraceMdcContext != null) {
				// overwrite if any
				fields.putAll(grpcTraceMdcContext.getContext());
			} else {
				// create tracing if not already present
				if (!fields.containsKey(GrpcTraceMdcContext.CORRELATION_ID_MDC_KEY)) {
					fields.putAll(GrpcTraceMdcContext.newBuilder().build().getContext());
				}
			}
		}
